...
expName = 'triggerDemo_pyxid2'  # from the Builder filename that created this script
expInfo = {
...
#generate a list of connected xid devices
devices = pyxid2.get_xid_devices()

#assign the first device in the list as 'dev'
#NB if multiple devices are connected (e.g., stimtracker & cedrus response box, this value may need to be changed)
dev=devices[0]
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/blake/Downloads/psychopy_biosemi_serial-main/triggerDemo_pyxid2.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    concl_text = visual.TextStim(win=win, name='concl_text',
        text='Thanks',
        font='Open Sans',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
    ...
    instructionsComponents = [instr_text]
    ...
    while continueRoutine and routineTimer.getTime() < 0.5:
        ...
        # if instr_text is stopping this frame...
        if instr_text.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > instr_text.tStartRefresh + .5-frameTolerance:
                # keep track of stop time/frame for later
                instr_text.tStop = t  # not accounting for scr refresh
                instr_text.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'instr_text.stopped')
                # update status
                instr_text.status = FINISHED
    ...
    for thisTrial in trials:
        ...
        #set the pule duration in ms
        dev.set_pulse_duration(15)
        
        #raise lines in the stimtracker to output unique triggers
        #the example below raises a line according to an input value
        dev.activate_line(lines=[trig])
        
        #lines can also be raised conditionally & in combination as in example below:
        #if condition=='audio_trial':
        #    dev.activate_line(lines=[1])
        #if condition=='video_trial':
        #    dev.activate_line(lines=[2])
        #if condition=='audiovisual_trial':
        #    dev.activate_line(lines=[1,2])
        # keep track of which components have finished
        trialComponents = [stim, trial_text]
        ...
        while continueRoutine and routineTimer.getTime() < 1.0:
            ...
            if trial_text.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > trial_text.tStartRefresh + .5-frameTolerance:
                    # keep track of stop time/frame for later
    ...
    while continueRoutine and routineTimer.getTime() < 0.5:
        ...
        # if concl_text is starting this frame...
        if concl_text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            concl_text.frameNStart = frameN  # exact frame index
            concl_text.tStart = t  # local t and not account for scr refresh
            concl_text.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(concl_text, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'concl_text.started')
            # update status
            concl_text.status = STARTED
            concl_text.setAutoDraw(True)
        
        # if concl_text is active this frame...
        if concl_text.status == STARTED:
            # update params
            pass
        
        # if concl_text is stopping this frame...
        if concl_text.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > concl_text.tStartRefresh + .5-frameTolerance:
                # keep track of stop time/frame for later
                concl_text.tStop = t  # not accounting for scr refresh
                concl_text.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'concl_text.stopped')
                # update status
                concl_text.status = FINISHED
                concl_text.setAutoDraw(False)
